...
def test_uc_decode_writes_the_decoded_instruction_to_the_printer():
    # ``sideeffects`` is a member like any other, so it shows up in the printed object;
    # neither of these decode blocks can flag one, so it is 0.
    assert _decode("05", 8) == (
        "0x05 FOO(encoding=<Encoding.T1: 1>, sideeffects=0, d=5)\n"
    )
...
def test_uc_decode_decodes_the_word_at_the_requested_size():
    ...
    assert _decode("1abc", 16) == (
        "0x1abc BAR(encoding=<Encoding.T1: 1>, sideeffects=0, n=2748)\n"
    )
