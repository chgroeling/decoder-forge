...
class BitPattern:
    ...
    def split_by_mask(self, mask: int) -> tuple["BitPattern", "BitPattern"]:
        ...
        Example:
            ...
            >>> pat_a, pat_b = orig.split_by_mask(0b101)
            >>> pat_a
    ...
    def extract_from_msb(self, bit_length: int) -> "BitPattern":
        ...
        greater than the current BitPattern's bit_length, a ValueError is raised.

        Args:
            bit_length (int): The number of bits for the extracted pattern. This value
              must not be greater than the current BitPattern's bit_length.
        ...
        Raises:
            ValueError: If the specified bit_length is greater than the current
              bit_length.
        ...
        if bit_length > self.bit_length:
            raise ValueError("BitPattern bit length is too big")
    ...
    def extend_and_shift_to_msb(self, bit_length: int) -> "BitPattern":
        ...
        if bit_length < self.bit_length:
            raise ValueError("BitPattern bit length is too small")
